...
export default function DetailKurikulum() {
  ...
  const navigate = useNavigate(); // <-- Tambahkan navigasi ini
  ...
  return (
    <div style={{ backgroundColor: 'var(--compreng-surface, #ffffff)', minHeight: '100vh', padding: '40px 20px', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <button 
          ...
          style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', fontSize: '14px', fontWeight: '600', color: 'var(--compreng-text-secondary, #475569)', transition: '0.2s', marginBottom: '40px', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}
        >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: '60px' }}>
          ...
          <h1 style={{ fontSize: '46px', fontWeight: '900', color: 'var(--compreng-text, #0f172a)', margin: '0 0 15px 0', letterSpacing: '-1px' }}>
            {current.title}
          </h1>
          <p style={{ fontSize: '18px', color: 'var(--compreng-text-secondary, #475569)', fontStyle: 'italic', margin: 0 }}>
            "{current.subtitle}"
          </p>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '50px', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: '1 1 60%', minWidth: '300px' }}>
            <h3 style={{ fontSize: '22px', fontWeight: '800', color: 'var(--compreng-text, #0f172a)', marginBottom: '15px' }}>Gambaran Umum</h3>
            <p style={{ fontSize: '15px', color: 'var(--compreng-text-secondary, #475569)', lineHeight: '1.8', marginBottom: '50px' }}>
              {current.desc}
            </p>

            <h3 style={{ fontSize: '22px', fontWeight: '800', color: 'var(--compreng-text, #0f172a)', marginBottom: '20px' }}>Mata Pelajaran Unggulan</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px' }}>
              {current.subjects.map((item, idx) => (
                <div key={idx} style={{ background: 'var(--compreng-bg, #f8fafc)', padding: '16px 20px', borderRadius: '12px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '12px', fontSize: '14px', fontWeight: '700', color: 'var(--compreng-text, #0f172a)', transition: '0.2s', cursor: 'default' }} onMouseOver={(e) => e.currentTarget.style.borderColor = current.badgeColor} onMouseOut={(e) => e.currentTarget.style.borderColor = '#e2e8f0'}>
          ...
          <div style={{ flex: '1 1 35%', minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '25px' }}>
            <div style={{ backgroundColor: 'var(--compreng-nav-bg, #0f172a)', borderRadius: '24px', padding: '40px 30px', color: 'var(--compreng-accent-text, #ffffff)', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', top: '-20px', right: '-20px', width: '100px', height: '100px', backgroundColor: 'var(--compreng-nav-bg, #0f172a)', borderRadius: '50%', opacity: '0.5' }}></div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', position: 'relative', zIndex: 1 }}>
                {current.career.map((car, idx) => (
                  <div key={idx} style={{ fontSize: '14px', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--compreng-text-secondary, #475569)' }}>
                    <Check size={16} color={current.badgeColor} style={{ flexShrink: 0 }} /> 
                    {car}
                  </div>
                ))}
              </div>
              <p style={{ fontSize: '12px', color: 'var(--compreng-text-secondary, #475569)', fontStyle: 'italic', marginTop: '30px', borderTop: '1px solid #334155', paddingTop: '20px', position: 'relative', zIndex: 1 }}>
